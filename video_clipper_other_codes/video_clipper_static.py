...
def get_interp_videos(opt):
    ...
    for item, videopath in enumerate(videolist):
        ...
        print('Saving folder:', opt.savepath)
        check_path(opt.savepath)
        savepath = os.path.join(opt.savepath, 'interp_' + str(item) + '_exposure_type_' + str(opt.exposure_type) + '.mp4')
        video = cv2.VideoWriter(savepath, fourcc, fps, (width, height))
            ...
            for i, interval in enumerate(interval_frame_list):
                if c == (interval - frame_half_crop_range):
                    ...
                    if discripancy > 0:
                        for dis in range(discripancy):
                            video.write(interp_frame)
                        print('This is the %d-th video %d-th interval. Finally %d frames are added' % (item + 1, i + 1, dis + 1))
            c = c + 1
            cv2.waitKey(1)
            rval, frame = vc.read()
        # release the video
...
def get_static_videos(opt):
    videolist = get_files(opt.video_folder_path)
    videolist = videolist[21:]
    print(videolist)
    for item, videopath in enumerate(videolist):
        # video statics
        print('Now it is the %d-th video with name %s' % (item + 1, videopath))
        fps, frames, time, width, height = vfc.get_video_info(videopath)
        fps = round(fps)
        width = opt.resize_w
        height = opt.resize_h
...
if __name__ == "__main__":
    ...
    parser.add_argument('--static_or_exposure', type = bool, default = True, help = 'True for static output; False for interp output')
    ...
    parser.add_argument('--videopath', type = str, \
        default = 'F:\\Deblur\\data collection\\video_original\\Moscow Russia Aerial Drone 5K Timelab.pro _ Москва Россия Аэросъемка-S_dfq9rFWAE.webm', \
            help = 'video path')
    ...
    parser.add_argument('--savepath', type = str, \
        default = 'F:\\Deblur\\data collection\\video_processed_v2', \
            help = 'save path')
